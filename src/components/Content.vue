                    ...
                    <DisclosurePanel class="pt-6">
                      <div class="space-y-6">
                        <div v-for="(option, optionIdx) in section.options" :key="option.value" class="flex items-center">
                          <input v-model="selectedFilters" :id="`filter-${section.id}-${optionIdx}`" :name="`${section.id}[]`" :value="option.value" type="checkbox" :checked="option.checked" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
      ...
      <main class="mx-auto max-w-max px-4 sm:px-6 lg:px-8">
        <div class="flex items-baseline justify-center md:justify-between border-b border-gray-200 pb-6 pt-24">
          ...
          <div class="flex items-center">
            ...
            <button @click="changeGrid()" type="button" class="-m-2 ml-5 p-2 text-gray-400 hover:text-gray-500 sm:ml-7">
        ...
        <section aria-labelledby="products-heading" class="pb-24 pt-10">
          <h2 id="products-heading" class="sr-only">Products</h2>
          <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
            <!-- Filters -->
            <form class="hidden lg:block lg:min-h-[50rem]">
              <h3 class="sr-only">Categories</h3>
            ...
            <!-- Product grid -->
            <div class="lg:col-span-3 min-w-[19rem] sm:min-w-[25rem] md:min-w-[42rem] lg:min-w-[45rem] xl:min-w-[65rem] 2xl:min-w-[80rem]">
              <ProductLists
                  :filteredProducts="filteredProducts"
                  :cssGridCols="cssGridCols"
              />
            </div>
          </div>
        </section>
...
import { mocked_sortOptions } from '../mocked-data/sortOptions.json'
import { mocked_subCategories } from '../mocked-data/subCategories.json'
import { mocked_filters } from '../mocked-data/filters.json'

const mobileFiltersOpen = ref(false)
const products = mocked_products
const sortOptions = mocked_sortOptions
const subCategories = mocked_subCategories
const filters = mocked_filters
const selectedFilters = ref(['beige', 'olive', 'white', 'gold', 'gray'])
...
const filteredProducts = computed(() => {
  ...
  selectedFilters.value.forEach((filter) => {
    newProducts = products.filter((p) => {
      return p.color.includes(filter)
    })
    newProducts.forEach((item) => {
      final.value.push(item)
    })
  })
...
let cssGridCols = ref('xl:grid-cols-4')
const changeGrid = () => cssGridCols.value = cssGridCols.value === 'xl:grid-cols-4' ? 'xl:grid-cols-2' : 'xl:grid-cols-4'
</script>